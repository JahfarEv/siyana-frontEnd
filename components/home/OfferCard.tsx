...
const OfferCard: React.FC<OfferCardProps> = ({
  ...
  const handleClick = () => {
    // Map offer subtitles to category slugs
    const offerToCategoryMap: Record<string, string> = {
      'Glowing Gold Rings': 'rings',
      'Glowing Gold Earrings': 'earrings',
      'Glowing Gold Necklace': 'pendant',
    ...
    if (categorySlug) {
      ...
      router.push('/category/rings'); // fallback
    }
  };
...
const OfferCards: React.FC = (): ReactElement | null => {
  const { data: OFFERS, isLoading, isError } = useOffers();

  if (isLoading || isError || !OFFERS?.length) {
    return null;
  }
  ...
  return (
    <section className="py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 px-4 sm:px-6 lg:px-8">
        {OFFERS.map((offer, index) => (
          <OfferCard title={''} subtitle={''} image={''} key={index} {...offer} />
        ))}
      </div>
    </section>
  );
